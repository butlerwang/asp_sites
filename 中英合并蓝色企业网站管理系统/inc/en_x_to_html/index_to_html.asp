...
'首页顶部幻灯广告读取替换
set rs=server.createobject("adodb.recordset")
sql="select top 5 name,url,image from en_web_ads  where [position]=36 and view_yes=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
...
WebAboutContent=left(ClearHtml(rs("content")),160)

select case rs("ppid")
case 1
ClassSQL="cid"
WebAboutFolder="/English/"&rs("folder") 
case 2
ClassSQL="pid"
...
rs0.open(sql),cn,1,1
if not rs0.eof then
WebAboutFolder="/English/"&rs0("folder")&"/"&rs("folder")
end if
rs0.close
set rs0=nothing
case 3
...
WebAboutFolder="/English/"&rs00("folder")&"/"&rs0("folder")&"/"&rs("folder") 
end if
rs00.close
set rs00=nothing
end if
...
case 1
ClassSQL="cid"
WebNewNewsFolder="/English/"&rs("folder") 
case 2
ClassSQL="pid"
set rs0=server.createobject("adodb.recordset")
sql="select [name],[folder],[id],[pid],[ppid] from [en_category] where id="&rs("pid")
rs0.open(sql),cn,1,1
if not rs0.eof then
WebNewNewsFolder="/English/"&rs0("folder")&"/"&rs("folder")
end if
rs0.close
set rs0=nothing
case 3
...
rs00.open(sql),cn,1,1
if not rs00.eof then
WebNewNewsFolder="/English/"&rs00("folder")&"/"&rs0("folder")&"/"&rs("folder") 
end if
rs00.close
set rs00=nothing
end if
...
set rs=server.createobject("adodb.recordset")
sql="select top 9 title,content,file_path,[url],time from [en_article]  where   "&ClassSQL&"='"&ItemID&"'  and view_yes=1  and ArticleType=1 order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof and not rs.bof then
for i=1 to 9
rs_url=""
...
WebNewNewsList=WebNewNewsList&"<tr><td width='75%' class='ListTitle'><a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'>"&left(rs("title"),42)&"</a></td>"
...
set rs=server.createobject("adodb.recordset")
sql="select top 8 [name],[folder],[id],[pid],[ppid] from [en_category] where pid="&ItemID&" and ClassType=2 and ppid=2 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to 8
...
sql="select top 12 [title],file_path,[image] from [en_article] where ArticleType=2 and cid='"&ItemID&"'  and pid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
Block03_LeftItem=Block03_LeftItem&"<DIV class='box'><A class='imgBorder'  href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><IMG alt='"&rsp("title")&"' src='/images/up_images/"&rsp("image")&"'><p>"&left(rsp("title"),24)&"</p></A> </DIV>"
rsp.movenext
loop
end if 
rsp.close
