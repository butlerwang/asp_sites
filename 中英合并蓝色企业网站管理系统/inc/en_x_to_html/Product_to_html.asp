...
sql="select top 8 [title],file_path,[image] from [en_article] where ArticleType=2 and id<>"&a_id&" and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
MorePro_List=MorePro_List&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><img src='/images/up_images/"&rsp("image")&"' width='139' height='139'/><div class='albumtitle'>"&left(rsp("title"),34)&"</div></a></div></div>"
rsp.movenext
loop
else
