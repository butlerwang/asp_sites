...
sql="select top 50 * from [en_article] where view_yes=1 order by [time] desc"
''根据自己实际修改
...
rs.open sql,cn,1,1   
 ...
 XMLContent=XMLContent&"<title>"& web_name & " </title>" & vbcrlf   
 XMLContent=XMLContent&"<link>" & web_url & "</link>" & vbcrlf   
 XMLContent=XMLContent&"<language>zh-cn</language>" & vbcrlf   
 XMLContent=XMLContent&"<copyright>RSS Feed By www.huiguer.com</copyright>" & vbcrlf    
while not rs.eof 
set rsc=server.createobject("adodb.recordset")
sqlc="select [name] from [en_category] where id="&rs("cid")
...
Content_FolderName=Product_FolderName
 ...
 XMLContent=XMLContent&"<link>"&web_url&"/"&Content_FolderName&"/"& rs("file_path") &"</link>" & vbcrlf    
...
call Generation_File("Feed.xml","English/rss",XMLContent) '在xml目录下生成编码为utf-8的Feex.xml文件
