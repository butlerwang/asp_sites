...
sql="select top 1 [id],cid from [en_Article] where [title]='"&a_title&"' and ArticleType=2 order by [time] desc"
rs2.open(sql),cn,1,1
if not rs2.eof  then
a_id=rs2("id")
ClassID=rs2("cid")
...
<%
response.Write "<script language='javascript'>alert('添加成功！');location.href='en_Product_list.asp';</script>"
end if 
