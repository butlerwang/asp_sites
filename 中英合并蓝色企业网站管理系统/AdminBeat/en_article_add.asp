...
if rs("url")="" then
name_url=web_url&"/Search/index.asp?q="&rs("name")
else
name_url=rs("url")
end if
...
sql="select top 1 [cid],[id] from [en_article] where [title]='"&a_title&"' and ArticleType=1 order by [time] desc"
rs2.open(sql),cn,1,1
if not rs2.eof  then
a_id=rs2("id")
ClassID=rs2("cid")
...
<%
response.Write "<script language='javascript'>alert('添加成功！');location.href='en_article_list.asp';</script>"
end if 
...
return true;}
	...
	<tr>
	  <td height=23 colspan="2" class='forumRow'><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td height="30" valign="top" class="TipWords"><p>1、本系统采用手动分页方法，如果想要对文章进行分页，只需在编辑器按钮栏点击<img src="images/inserthorizontalrule.gif" width="20" height="20" />图标，就会在编辑器内自动插入一条线做为分页标志，你不需要删除它，它将不会在文章中显示。</p></td>
        </tr>
        <tr>
          <td height="10">&nbsp;</td>
        </tr>
...
%>
	  ...
	  <tr>
	    ...
	    <td width="85%" class='forumRowHighLight'><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
           <td ><input id="Files" name="Files" type="text" size="80" /> <br><input type="button" value="上传文件" onClick="showUpload(null,'Files','',10,null);" />
		   </td>
         </tr>
       </table></td>
      </tr>
