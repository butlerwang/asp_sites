...
sql="select article_id from web_product_comment where id="&cint(article_id(i))&""
rs.open(sql),cn,1,3
a_id=rs("article_id")
rs.delete
rs.close
...
next
			...
			set rs=server.createobject("adodb.recordset")
sql="select id,article_id from web_product_comment where id="&article_id&""
rs.open(sql),cn,1,3
a_id=rs("article_id")
rs.delete
rs.close
...
%>
<%
response.Write "<script language='javascript'>alert('删除成功！');location.href='Pcomment_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
			%></td>
          </tr>
        </table>
	    </td>
