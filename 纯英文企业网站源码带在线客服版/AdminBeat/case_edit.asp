...
%>
<%
response.Write "<script language='javascript'>alert('OK');location.href='case_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
...
set rsc2=server.createobject("adodb.recordset")
sqlClass2="select id,pid,ppid,name from category where ppid=2 and ClassType=3 and pid="&cint(rs("cid"))&" order by id" 
rsc2.open sqlClass2,cn,1,1
...
%>
			...
			if rs("pid")<>"" then				
set rsc3=server.createobject("adodb.recordset")
sqlClass3="select id,pid,ppid,name from category where ppid=3 and ClassType=3 and pid="&cint(rs("pid"))&" order by id" 
rsc3.open sqlClass3,cn,1,1
	    ...
	    <td width="85%" class='forumRowHighLight'><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
           <td ><input id="Pics" name="Pics" type="text" size="80" /> <br><input type="button" value="上传图片" onClick="showUpload2(null,'Pics','',100,null);" />
		           ...
		           <tr>
           <td >
           <iframe width="500" name="ad" frameborder=0 height="100" scrolling=yes src="Pics_list.asp?id=<%=rs("id")%>"></iframe>
		   </td>
         </tr>         
       </table></td>
      </tr>
