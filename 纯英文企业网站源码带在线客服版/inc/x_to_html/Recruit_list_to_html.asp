...
set rs=nothing
set rs=server.createobject("adodb.recordset")
sql="select top 8 title,content,file_path,[url],time from [article]  where  cid='"&NewsID&"'  and view_yes=1  and ArticleType=1 order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof and not rs.bof then
...
Block01_LeftItem=Block01_LeftItem&"<dd><a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'>"&left(rs("title"),24)&"</a></dd>"
rs_0.close
set rs_0=nothing
rs.movenext
next
...
list_block=list_block&"<tr><td width='14%' align='center' bgcolor='#F9F9F9'>Conditions：</td>"
...
<%'循环列表替换内容
replace_code=replace(replace_code,"$list_block$"," ")   
