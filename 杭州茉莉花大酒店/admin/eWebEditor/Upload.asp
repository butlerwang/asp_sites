...
'☆                                                                  ☆
'★  中国美术学院美术中心地址: http://www.eWebSoft.com/   eWebSoft团队及产品         ★
...
Sub DoSave()
  ...
  Case "0"
   ' 默认无组件上传类
                    Call DoUpload_Class
  Case "1"
     Call DoUpload_AspLoad()
  Case "2"
      Call DoUpload_SA_FileUp()
  Case "3"
...
Sub DoUpload_LyfUpload()
 Dim oUpload,sFileName,oFile
 dim yfilename,path,varfname,ss
   on Error Resume Next
 Set oUpload = Server.CreateObject("LyfUpload.UploadFile")
   oUpload.maxsize = nAllowSize*1024
    oUpload.strway = false
  sFileName= oUpload.SaveFiletodb("uploadfile")
  sFileExt = Mid( sFileName,InstrRev( sFileName,".")+1)
 Call CheckValidExt(sFileExt)
 sSaveFileName = GetRndFileName(sFileExt)
 sOriginalFileName =  oUpload.Filename
 ss=oUpload.SaveFile(yFileName, Server.Mappath(sUploadDir), false,sSaveFileName)
 Set oUpload=nothing
End Sub

''===========================SA-FileUp 4.0组件上传============================
Sub  DoUpload_SA_FileUp()
   Dim Count, sFileName
      Dim oUpload, oFile
      on Error Resume Next
 ...
 oUpload.IgnoreNoPost = True
 oUpload.SetMaxSize nAllowSize*1024, True
 sFileName= oUpload.UserFileName
 sFileExt = GetMyFileExt(sFileName)
 Call CheckValidExt(sFileExt)
 sSaveFileName = GetRndFileName(sFileExt)
 sOriginalFileName =  oUpload.UserFilename
 oUpload.SaveAs Server.Mappath(sUploadDir & sSaveFileName )
...
'========AspUpload3.0组件上传=========
Sub DoUpload_AspLoad()
   Dim Count, sFileName
   Dim oUpload, oFile
   on Error Resume Next

   Set oUpload = Server.CreateObject("Persits.Upload")
   oUpload.OverwriteFiles = False   ' 不能复盖
   oUpload.IgnoreNoPost = True
   oUpload.SetMaxSize nAllowSize*1024, True
   Count = oUpload.Save
...
' 获取文件扩展名
Function GetMyFileExt(FileName)
    Dim s
    s = Split(FileName, ".")
    If UBound(s) < 1 Then
        GetMyFileExt = ""
    Else
        GetMyFileExt = LCase(s(UBound(s)))
    End If
End Function
' ===============================
