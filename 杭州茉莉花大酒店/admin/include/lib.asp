...
Set rs_f=Server.CreateObject("adodb.recordset")
  Sql_f="Select ParPath,ClassName,ID from "&TableName&" Where ID="&ClassID
  rs_f.Open Sql_f,Conn,1,1
  if rs_f.eof then Exit Function
     If rs_f(0)=0 Then
		 if cint(lianjie)=1 then
		    FatherNameS="<a href='"&weblink&"?ClassID="&rs_f(2)&"' title='"&rs_f(1)&"' class='"&ziti&"'>"&rs_f(1)&"</a>" 
		   else 
		    FatherNameS=rs_f(1)
		  end if
	 ...
	 Else
		 ...
		 'SQL="Select ID From "&TableName&" Where ID in ("&rs_f(0)&","&ClassID&")"
		 SQL="Select ID From "&TableName&" Where ID in ("&rs_f(0)&")"
		 'response.Write SQL
...
'返回值：√ or ×
 ...
 sqlt="Select Show from Sbe_Product_Field where FieldTitle='"&content&"'"
' response.Write sqlt
' response.End
 Rst.Open Sqlt,Conn,1,1
  if not(rst.eof and rst.bof) then
    Judgement2=Judgement3(rst(0))
  end if
 rst.close:Set rst=nothing
end Function
Function Judgement3(content) '函数的建立
'函数名：judgement 
'作  用：判断是否。
'参  数：content---判断内容
'返回值：√ or ×
if content=true then
   Judgement3="style=""display:block;"""
  else 
   Judgement3="style=""display:none;"""
  end if
end Function
...
set rs_w=server.CreateObject("adodb.recordset")
 sql_w="select * From Sbe_WebConfig "
 rs_w.open sql_w,conn,1,1
   NewsClass_num=rs_w("NewsClass_num")    '新闻分类级数
   ProClass_num=rs_w("ProClass_num")      '产品分类级数
   downClass_num=rs_w("downClass_num")    '下载分类级数
   qiyeClass_num=rs_w("qiyeClass_num")      '企业信息分类级数
   banben=rs_w("banben")   '版本: 1-中文版 , 2-中英文版
   if banben=1 then banben_display="style=""display:none;"""
   hy_message=rs_w("hy_message")       '留言板是否有回复
   sf_yingpin=rs_w("sf_yingpin")       '在线招聘是否应聘
   yanzhengma=rs_w("yanzhengma")       '后台登录是否有验证码
   weblink_leibie=rs_w("weblink_leibie") '友情链接类型
   if weblink_leibie<>2 then weblink_display1="style=""display:none;"""
   if sf_yingpin=false then yingpin_display="style=""display:none;"""
   if sf_yingpin=false then
      web_yingpin ="list.asp"
	 else
	  web_yingpin ="resume.asp"
	end if
 rs_w.close
set rs_w=nothing
Call CloseDataBase()
