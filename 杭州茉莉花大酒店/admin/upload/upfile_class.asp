...
Public Sub GetData (MaxSize)
	...
	if isGetData_=false then 
		Dim RequestBinDate,sSpace,bCrLf,sInfo,iInfoStart,iInfoEnd,tStream,iStart,xingjilz
		...
		Do
			...
			If InStr (45,sInfo,"filename=""",1) > 0 Then
				Set xingjilz = new FileInfo_Class
				'取得文件属性
				iFindStart = InStr (iFindEnd,sInfo,"filename=""",1)+10
				iFindEnd = InStr (iFindStart,sInfo,""""&vbCrLf,1)
				sFileName = Trim(Mid(sinfo,iFindStart,iFindEnd-iFindStart))
				xingjilz.FileName = GetFileName(sFileName)
				xingjilz.FilePath = GetFilePath(sFileName)
				xingjilz.FileExt = GetFileExt(sFileName)
				iFindStart = InStr (iFindEnd,sInfo,"Content-Type: ",1)+14
				iFindEnd = InStr (iFindStart,sInfo,vbCr)
				xingjilz.FileMIME = Mid(sinfo,iFindStart,iFindEnd-iFindStart)
				xingjilz.FileStart = iInfoEnd
				xingjilz.FileSize = iFormStart -iInfoEnd -2
				xingjilz.FormName = sFormName
				file.add sFormName,xingjilz
			else
			'如果是表单项目
				tStream.Close
				tStream.Type = 1
