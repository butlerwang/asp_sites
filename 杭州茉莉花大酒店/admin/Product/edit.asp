...
'		Set Rs1=Nothing
	...
	rs.Close
	Set Rs=Nothing 
    response.Write("<script language=javascript>alert('客房信息修改成功！');window.location.href='"&request.Form("returnurl")&"';</script>")
	response.End()
  End Sub
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr> 
    <td height="25"><font color="#6A859D">客房中心 &gt;&gt; 修改客房信息</font></td>
  </tr>
  <tr>
...
<!--    <tr <%'Call OpenClose(rs(3))%>  style="display:none" > 
    ...
    <tr <%Call OpenClose(rs(3))%>  style="display:none"> 
      ...
      <td colspan="2"> <input type="radio" name="Tuijian" value="1"  <%Call ReturnSel(rs2("Tuijian"),true,2)%>>
        是 &nbsp;&nbsp; <input name="Tuijian" type="radio" value="0"  <%Call ReturnSel(rs2("Tuijian"),false,2)%>>否</td>
    </tr>
  ...
  <tr id="show_user_rights" <%'if Tuijian=false then response.write("style='display:none;'") end if%>>
    ...
    <tr <%if rs(3)=true then%><%=banben_display%><%else%><%Call OpenClose(rs(3))%><%end if%>> 
      ...
      <td colspan="2"> <input type="radio" name="leibie" value="1" <%Call ReturnSel(rs2("leibie"),1,2)%>>
        中 &nbsp;&nbsp; <input name="leibie" type="radio" value="2"  <%Call ReturnSel(rs2("leibie"),2,2)%>>
        英</td>
    </tr>
<tr  style="display:none"> 
      ...
      <td colspan="2"> <input type="radio" name="shifou" value="0" checked="checked" onClick="PasswordShow(0)" <%Call ReturnSel(rs2("shifou"),false,2)%>>
        不需用户名 &nbsp;&nbsp; <input name="shifou" type="radio" value="1" onClick="PasswordShow(1)" <%Call ReturnSel(rs2("shifou"),true,2)%>>
        需要用户名<span id="Showpassword" <%if rs2("shifou")=false then response.Write("style='display:none'") end if%>>&nbsp;&nbsp;
...
<!--    <tr <%Call OpenClose(rs(3))%> style="display:none"> 
    ...
    <tr <%Call OpenClose(rs(3))%>>
      <td height="13" align="center">数量</td>
...
<input type="radio" name="<%=FieldName%>" value="1" <%Call ReturnSel(FieldValue,true,2)%>>
        是 &nbsp;&nbsp; <input name="<%=FieldName%>" type="radio" value="0"  <%Call ReturnSel(FieldValue,false,2)%>>
        否	  
<%
