...
sql="select web_name,web_url,web_image,web_title,web_keywords,web_person,web_tel,web_TopHTML,web_BottomHTML,web_description,web_copyright,web_theme from en_web_settings"
rs.open(sql),cn,1,1
...
WebAboutContent=left(ClearHtml(rs("content")),70)
...
'新闻动态
set rs=server.createobject("adodb.recordset")
sql="select top 3 [name],[folder],[id],[pid],[ppid] from [en_category] where ClassType=1 and ppid=1 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
...
sql="select top 7 [title],file_path,[content],[time] from [en_article] where  cid='"&rs("id")&"'  order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
...
Block033_LeftItem=Block033_LeftItem&"<tr><td width='75%'>· <a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),45)&"</a></td>"
...
set rs=nothing
set rs=server.createobject("adodb.recordset")
sql="select top 8 [name],[folder],[id],[pid],[ppid] from [en_category] where pid="&ItemID&" and ClassType=2 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to 8
 if i=1 then
...
set rsp=server.createobject("adodb.recordset")
sql="select top 4 [title],file_path,[image] from [en_article] where ArticleType=2 and pid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
Block03_LeftItem=Block03_LeftItem&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><img src='/images/up_images/"&rsp("image")&"' width='166' height='166'/><div class='albumtitle'>"&left(rsp("title"),15)&"</div></a></div></div>"
rsp.movenext
loop
end if 
rsp.close
