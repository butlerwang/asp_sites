...
sql="select * from en_web_ads_position where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("memo")=l_memo
rs.update
rs.close
set rs=nothing
response.Write "<script language='javascript'>alert('修改成功！');location.href='en_ads_position_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from en_web_ads_position where id="&article_id&""
rs.open(sql),cn,1,1
...
return true;}
	...
	<tr>
	  <th class='tableHeaderText' colspan=2 height=25>修改广告位置</th>
	<tr>
