...
if act="search" then
if keywords<>"" then
s_sql="select * from en_web_article_comment where [content] like '%"&keywords&"%'  and article_id<>0 order by [time] desc "
else
s_sql="select * from en_web_article_comment where [content] like '%"&keywords&"%' and article_id<>0 order by [time] desc"
end if
else
...
%>
 ...
 <form name="form2" method="post" action="en_Comment_Del.asp?action=AllDel&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">
		...
		<table width="95%" border="0" align="center" cellpadding="0" cellspacing="2">
          <tr>
            ...
            <td width="52%" height="30" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">评论内容</div></td>
            <td width="8%" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">评论者</div></td>
            <td width="7%" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">审核</div></td>
            <td width="18%" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">评论时间</div></td>
            <td width="9%" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">文章操作</div></td>
          </tr>
<% '文章列表模块
strFileName="en_Comment_list.asp" 
pageno=20
...
%>
          ...
          <tr >
            ...
            <td height="30" class='forumRowHighLight' ><span style="color: #FF0000">文章：</span>
            ...
            <td class='forumRowHighLight'><div align="center"><a href="en_Comment_view_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已审核<%else%><span style="color: #FF0000">未审核</span><% end if%></a></div></td>
            <td class='forumRowHighLight' ><div align="center"><%=rs("time")%></div></td>
            <td class='forumRowHighLight' >
            <div align="center"><a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_Comment_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a></div>				</td>
          </tr>
		            <tr >
            <td height="50" colspan="5" valign="top" class='forumRow'  style="line-height:180%"><span style="color: #FF0000">评论：</span><%=rs("content")%></td>
          </tr>
