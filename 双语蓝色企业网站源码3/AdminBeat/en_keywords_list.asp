...
if act="search" then
if keywords<>"" then
s_sql="select * from en_web_keywords where [name] like '%"&keywords&"%' order by time desc "
else
s_sql="select * from en_web_keywords where [name] like '%"&keywords&"%' order by time desc"
end if
else
...
%>
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="en_keywords_add.asp">添加新的关键词</a></td>
          </tr>
          <tr>
            <td height="30"></td>
          </tr>
...
<% '文章列表模块
strFileName="en_keywords_list.asp" 
pageno=25
...
class_style="forumRowHighLight"
          ...
          <tr >
           ...
           <td class='<%=class_style%>' ><div align="center"><%=rs("name")%></div></td>
            ...
            <td class='<%=class_style%>' ><div align="center"><a href="en_keywords_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("hot_yes")=1 then%><span style="color: #FF0000">热门</span><%else%>非热门<% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><a href="en_keywords_index.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("index_yes")=1 then%><span style="color: #FF0000">框内</span><%else%>非框内<% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><%=rs("time")%></div></td>
            <td class='<%=class_style%>' >
            <div align="center"><a href="en_keywords_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_keywords_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
		  next
