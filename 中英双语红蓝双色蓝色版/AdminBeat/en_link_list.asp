...
if act="search" then
if keywords<>"" then
s_sql="select * from en_web_link where [name] like '%"&keywords&"%' order by [order] "
else
s_sql="select * from en_web_link where [name] like '%"&keywords&"%' order by [order] "
end if
else
...
sql="select id,order from en_web_link where id="&id1&""
rs1.open(sql),cn,1,3
...
%>
	...
	<tr>
	  <th width="100%" height=25 class='tableHeaderText'>链接列表</th>
	
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="en_link_add.asp">添加新的链接</a></td>
          </tr>
          <tr>
            <td height="30"></td>
          </tr>
...
class_style="forumRowHighLight"
          ...
          <tr >
		   ...
		   %></div></td>
            ...
            <td class='<%=class_style%>' ><div align="center"><a href="en_link_view_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已审核<%else%><span style="color: #FF0000">未审核</span><% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><%=rs("time")%></div></td>
            <td class='<%=class_style%>' >
            <div align="center"><a href="en_link_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_link_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
		  next
