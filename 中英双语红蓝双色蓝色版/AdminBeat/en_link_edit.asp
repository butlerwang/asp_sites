...
sql="select * from en_web_link where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("url")=l_url
rs("image")=l_image
...
set rs=nothing
call index_to_html()
response.Write "<script language='javascript'>alert('修改成功！');location.href='en_link_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from en_web_link where id="&article_id&""
rs.open(sql),cn,1,1
...
return true;}
	...
	<tr>
	  <th class='tableHeaderText' colspan=2 height=25>修改链接</th>
	<tr>
