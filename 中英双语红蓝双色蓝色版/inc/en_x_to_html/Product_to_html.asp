...
Block01_LeftItem=Block01_LeftItem&"<dd><a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'>"&left(rs("title"),64)&"</a></dd>"
rs_0.close
set rs_0=nothing
rs.movenext
next
...
MorePro_List=MorePro_List&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><img src='/images/up_images/"&rsp("image")&"' width='139' height='139'/><div class='albumtitle'>"&left(rsp("title"),24)&"</div></a></div></div>"
rsp.movenext
loop
else
...
sql="select id from [en_web_order] where [article_id]="&a_id&" "
rso.open(sql),cn,1,1
...
set rs=server.createobject("adodb.recordset")
sql="select  [name],[folder] from [en_category] where ppid=3 and pid="&rsl("id")&" "
rs.open(sql),cn,1,1
if not rs.eof then
