...
Class DIYFunction
		...
		Private datasourcetype,datasourcestr,ajax,FileName
		...
		ElseIf KS.G("action")="testfilename" then
		  call testfilename():exit sub
		end if
	  ...
	  sub testlabelname()
			...
			if labelid<>"" then 
			 RS.Open "Select top 1 id From [KS_Label] Where id<>'" & labelid & "' and LabelName='" & "{XML_" & LabelName & "}" & "'", Conn, 1, 1
			else
			 RS.Open "Select top 1 id From [KS_Label] Where LabelName='" & "{XML_" & LabelName & "}" & "'", Conn, 1, 1
			end if
			If Not RS.EOF Then
			 KS.Echo "false"
			Else
	  ...
	  sub testfilename()
	        Dim LabelID:LabelID=request.QueryString("labelid")
			If  FileName ="" Then KS.Die "false"
			If right(lcase(FileName),4)<>".xml" Then KS.Die "false"
			Dim RS:Set RS = Server.CreateObject("Adodb.RecordSet")
			if labelid<>"" then 
			 RS.Open "Select top 1 id From [KS_Label] Where id<>'" & labelid & "' and fileName='" &  FileName  &  "'", Conn, 1, 1
			else
			 RS.Open "Select top 1 id From [KS_Label] Where fileName='" & FileName & "'", Conn, 1, 1
			end if
			If Not RS.EOF Then
			 KS.Echo "false"
			Else
	  ...
	  Sub Step0()
		...
		If LabelID <> "" Then
			...
			If Not LabelRS.Eof Then
				LabelName    = Replace(Replace(LabelRS("LabelName"), "{XML_", ""), "}", "")
				FileName     = LabelRS("FileName")
 ...
 %>
	    <script type="text/javascript">
		  function CheckForm()
		  {
		   ...
		   if ($("#FileName").val().toLowerCase().indexOf('.xml')==-1)
		   {
		      alert('XML文件名必须以.xml为扩展名!');
			  $('#FileName').focus();
			  return;
		   }
		  ...
		  var url = 'LabelXML.asp';
  		  $.get(url,{action:"testfilename",labelid:"<%=labelid%>",filename:FileName},function(d){
		    if (d=='true')
			  $('#filenamemessage').html('<font color=blue>恭喜，可以使用该文件名!</font>');
			else
			  $('#filenamemessage').html('<font color=red>对不起，该文件名不可用,请重输!</font>');
			  $('#filenametf').val(d);
		  });
		  }
		...
		<br>
	     <table border='0' cellspacing='1' cellpadding='1' width='95%' align='center' class='ctable'>
		  <form action="?action=<%=ActionStr%>" method="post" id="myform" name="myform">
		   ...
		   <input name='filenametf' id='filenametf' type='hidden'>
		   <input type='hidden' name='labelid' value='<%=labelid%>'>
		  <tr class='tdbg'>
		    <td class='clefttitle' align='right'><strong>XML名称:</strong></td>
		    <td><input name="LabelName" id="LabelName" value="<%=LabelName%>" onblur='testlabelname()' style="width:200px;"> <font color=red>*</font> <span class="tips">例如百度新闻XML,幻灯片调用XML等。</span><span id='labelmessage'></span></td>
		  </tr>
		  <tr class='tdbg'>
		    <td class='clefttitle' align='right'><strong>生成XML文件名:</strong></td>
		    <td class="tips"><%=KS.Setting(127)%><input name="FileName" id="FileName" value="<%=FileName%>" onblur='testfilename()' style="width:200px;"> <font color=red>*</font> 例如news.xml,config.xml等。</font><span id='filenamemessage'></span></td>
		  </tr>
		  <tr class='tdbg'>
		   <td width="80" height="30" class='clefttitle' align='right'><strong>标签目录:</strong></td>
		   <td><%=ReturnLabelFolderTree(FolderID, 7)%><font color=""#FF0000"">请选择标签归属目录，以便日后管理标签</font></td>
		  </tr>
		  <tr class='tdbg'>
	  ...
	  function CheckForm()
		{ var form=document.myform; 
		 ...
		 function changedb()
		 {
		  ...
		  var Note=$('#Note').val();
		  location.href='LabelXML.asp?action=Step1&Flag=addfield&FileName='+FileName+'&Note='+Note+'&datasourcetype='+datasourcetype+'&datasourcestr='+datasourcestr+'&dbname1='+dbname1+'&dbname2='+dbname2+'&LabelName='+LabelName+'&ParentID='+ParentID+'&Page='+Page+'&LabelID='+LabelID+'&SQLType='+SQLType+'&ItemName='+ItemName;
		 }
		function addfield(){
		...
		<table style="margin-top:5px" width="100%" border=0 cellpadding='2' cellspacing='1' class='border'>
		  ...
		  <%
			...
			<tr class="tdbg">
			  ...
			  <td width=100>
			<Select class="textbox" style="WIDTH: 250px; HEIGHT: 250px" onChange='addfield()' multiple size=1 name="field">
			<%
			...
			%>
			  ...
			  <td>
		<Select class="textbox" style="WIDTH: 250px; HEIGHT: 250px" onChange='addfield()' multiple size=2 name="field2">
		  <%
	      ...
	      <tr class="tdbg">
            <td width='100' align='center'><strong>SQL查询语句：</strong></td>
	...
	End Function
	 ...
	 Sub Step2()
		...
		Else
		  ...
		  LabelContent="<?xml version=""1.0"" encoding=""utf-8""?>"&vbcrlf&"<root>" & vbcrlf &" [loop=" & LoopTimes &"]" & vbcrlf & "  请在此输入循环内容" & vbcrlf &" [/loop]" & vbcrlf &"</root>"&vbcrlf
		End If
		...
		.echo "  <form name=""myform"" id=""myform"" method=post action=""LabelXML.asp"">"
		...
		.echo "第三步：构造XML样式（循环内容）</font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:ShowIframe()'><u>查看栏目<=>ID对照表</u></a>"
		.echo " </div></td>"
		.echo "    </tr>"
		...
		.echo "      <td height=""16"" style=""text-align:center;font-weight:bold"">查询语句</td>"
		.echo "      <td><textarea name=""LabelIntro"" rows=""2"" style=""width:98%;"">" & LabelIntro & "</textarea></td>"
		.echo "    </tr>"
		.echo "    <tr class=""tableBorder1"">"
		.echo "      <td height=""16"" style=""text-align:center;font-weight:bold"">XML文件名</td>"
		.echo "      <td class=""tips""><input type=""hidden"" name=""FileName"" id=""FileName"" value=""" & FileName &""" ><input type=""text"" name=""FileName1"" id=""FileName1"" value=""" & FileName &""" disabled> 不可改</td>"
		.echo "    </tr>"
        .echo "    <tr class=""tableBorder1"">"
		.echo "      <td  height=""30"" nowrap style=""text-align:center""><strong>可用字段</strong></td>"
		.echo "      <td>"
...
Sub AddLabelSave()
			...
			ItemName=KS.G("ItemName")
			If LabelName = "" Then
			   Call KS.AlertHistory("XML名称不能为空!", -1)
			   Set KS = Nothing
			   Exit Sub
			End If
			...
			If LabelContent = "" Then
			  ...
			  Exit Sub
			End If
			LabelName = "{XML_" & LabelName & "}"
			Set LabelRS = Server.CreateObject("Adodb.RecordSet")
			LabelRS.Open "Select top 1 LabelName From [KS_Label] Where LabeLFlag=7 and LabelName='" & LabelName & "'", Conn, 1, 1
			If Not LabelRS.EOF Then
			  Call KS.AlertHistory("XML名称已经存在!", -1)
			  LabelRS.Close
			  Conn.Close
			  Set LabelRS = Nothing
			  Set Conn = Nothing
			...
			Else
				...
				LabelRS.AddNew
				 ...
				 LabelRS("Description") = Descript &"@@@"&FieldParam&"@@@"&SQLType&"@@@"&ItemName&"@@@@@@@@@"& datasourcetype &"@@@" &datasourcestr & "@@@" & note
				 LabelRS("FolderID") = FolderID
				 LabelRS("FileName") = FileName
				 LabelRS("AddDate") = Now
				 LabelRS("LabelType") = 7
				 LabelRS("OrderID") = 1
				 LabelRS.Update
				 Call KS.FileAssociation(1021,2,LabelContent,0)
				  Call CreateXML(LabelID)
				 KS.echo ("<script>if (confirm('成功提示:\n\n添加生成XML文档参数成功,继续添加吗?')){location.href='LabelXML.asp?Action=AddNew&LabelType=7&FolderID=" & FolderID & "';}else{$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & FolderID & "&OpStr=标签管理 >> 自义定生成XML文档&ButtonSymbol=DIYFunctionLabel';parent.frames['MainFrame'].location.href='Label_Main.asp?LabelType=7&FolderID=" & FolderID & "';}</script>")
			End If
	End Sub
	...
	Sub EditLabelSave()
			...
			Else
				...
				LabelRS.Open "Select top 1 * From [KS_Label] Where ID='" & LabelID & "'", Conn, 1, 3
				 ...
				 LabelRS.Close
				 LabelRS.Open "Select LabelContent From KS_Label Where LabelType=7",conn,1,1
				 ...
				 If KeyWord = "" Then
					KS.Echo ("<script>alert('成功提示:\n\nXML文档参数修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?LabelFolderID=" & FolderID & "&OpStr=标签管理  >> 自定义函数标签&ButtonSymbol=DIYFunctionLabel';location.href='Label_main.asp?Page=" & Page & "&LabelType=7&FolderID=" & FolderID & "';</script>")
				 Else
					KS.Echo ("<script>alert('成功提示:\n\nXML文档参数修改成功!');$(parent.document).find('#BottomFrame')[0].src='" & KS.Setting(3) & KS.Setting(89) & "KS.Split.asp?OpStr=标签管理 >> <font color=red>搜索自定义生成XML文档结果</font>&ButtonSymbol=DIYFunctionSearch';location.href='Label_main.asp?Page=" & Page & "&LabelType=7&KeyWord=" & KeyWord & "&SearchType=" & SearchType & "&StartDate=" & StartDate & "&EndDate=" & EndDate & "';</script>")
				 End If
			End If
	End Sub
