...
WebAboutContent=left(ClearHtml(rs("content")),95)
...
WebNewNewsList=WebNewNewsList&"<dd><a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'>"&left(rs("title"),18)&"</a></dd>"
rs_0.close
set rs_0=nothing
rs.movenext
next
...
set rs=nothing
set rs=server.createobject("adodb.recordset")
sql="select top 6 [name],[folder],[id],[pid],[ppid] from [category] where pid="&ItemID&" and ClassType=2 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to 6
 if i=1 then
...
Block033_LeftItem=Block033_LeftItem&"<p>"&left(rsp("title"),13)&"</p>"
