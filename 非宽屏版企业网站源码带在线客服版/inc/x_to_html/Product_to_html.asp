...
sql="select top 6 [title],file_path,[image] from [article] where ArticleType=2 and id<>"&a_id&" and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
MorePro_List=MorePro_List&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><img src='/images/up_images/"&rsp("image")&"'  title='"&left(rsp("title"),139)&"'/></a><div class='albumtitle ColorLink'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),16)&"</a></div></div></div>"
rsp.movenext
loop
else
...
article_short=left(rs("description"),40)&"..."
