...
if cid=""  then
s_sql="select * from web_menu where [name]  like '%"&keywords&"%'  order by [position],[order]"
else
search_sql="and [position]="&cid&""
s_sql="select * from web_menu where [name] like '%"&keywords&"%'"&search_sql&"  order by [position],[order]"
end if

else
s_sql="select * from web_menu order by [position],[order]"
