	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td height="30" valign="top" class="TipWords">
              <p>1、目前可以设置最高三级栏目，点击栏目名称色块处即可看到它的下级栏目。</p>
              <p>2、栏目属性分为：单页、文章、产品、案例、扫聘五种。</p></td>
          </tr>
          <tr>
            <td height="10" ></td>
          </tr>
...
sql="select id,pid,ppid,name,folder,ClassType,[order] from category where ppid=1 order by ClassType,[order],time"
rs.open(sql),cn,1,1
...
for p_i=1 to loopno
          ...
          <tr >
			...
			case 3
			response.write "[案例] "
			ListName="case_list.asp"
			case 4
			response.write "[招聘] "
			ListName="Info_list.asp"			
			case 5
			response.write "[单页] "
			ListName="category_edit.asp?id="&rs("id")&"&ppid=1&page="&page&"&act="&act&"&keywords="&keywords&""						
			end select%>
			一级栏目            </div></td>
            <td class="TitleHighlight2" >
            <div align="center"><a href="category_add.asp?pid_name=<%=rs("name")%>&pid=<%=rs("id")%>&ppid=2&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>&ClassTypeID=<%=rs("ClassType")%>">添加二级栏目</a> | <a href="category_edit.asp?id=<%=rs("id")%>&ppid=1&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">栏目设置</a> | <a href="<%=ListName%>?cid=<%=rs("id")%>&act=search">内容管理</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='category_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除栏目</a>            </div></td>
          </tr>
		    <tr id="class_<%=rs("id")%>" >
...
if not rs2.eof and not rs2.bof then
		  ...
		  %>
			...
			case 5
			response.write "[单页] "
			ListName="category_edit.asp?id="&rs2("id")&"&pid_name="&rs("name")&"&ppid=2&"						
			end select%>二级栏目            </div></td>
            <td width="42%" class="TitleHighlight3"  >
              <div align="center"><a href="category_add.asp?pid_name=<%=rs("name")%>&pid_name2=<%=rs2("name")%>&pid=<%=rs2("id")%>&ppid=3&ClassTypeID=<%=rs2("ClassType")%>"">添加三级栏目</a> | <a href="category_edit.asp?id=<%=rs2("id")%>&pid_name=<%=rs("name")%>&ppid=2">栏目设置</a> | <a href="<%=ListName%>pid=<%=rs2("id")%>&act=search">内容管理</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='category_del.asp?id=<%=rs2("id")%>';">删除栏目</a> </div>			  </td></tr>
...
if not rs3.eof and not rs3.bof then
		  ...
		  %>
			...
			case 5
			response.write "[单页] "
			ListName="category_edit.asp?id="&rs3("id")&"&pid_name="&rs("name")&"&pid_name2="&rs2("name")&"&ppid=3&"						
			end select%>三级栏目            </div></td>
            <td width="42%" class='forumRowHighlight'  >
              <div align="center"><a href="category_edit.asp?id=<%=rs3("id")%>&pid_name=<%=rs("name")%>&pid_name2=<%=rs2("name")%>&ppid=3">栏目设置</a> | <a href="<%=ListName%>cid=<%=rs3("id")%>&act=search">内容管理</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='category_del.asp?id=<%=rs3("id")%>';">删除栏目</a> </div>			  </td></tr>
					  <%
		  rs3.movenext
		  loop 
else
