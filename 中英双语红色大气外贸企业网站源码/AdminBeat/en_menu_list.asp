...
if cid=""  then
s_sql="select * from en_web_menu where [name]  like '%"&keywords&"%'  order by [order]"
else
search_sql="and [position]="&cid&""
s_sql="select * from en_web_menu where [name] like '%"&keywords&"%'"&search_sql&"  order by [order]"
end if

else
s_sql="select * from en_web_menu order by [order]"
...
sql="select id,order from en_web_menu where id="&id1&""
rs1.open(sql),cn,1,3
rs1("order")=order1
rs1.update
rs1.close
...
%>
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="en_menu_add.asp">添加新的二级航</a></td>
          </tr>
          <tr>
            <td height="30"></td>
          </tr>
...
class_style="forumRowHighLight"
          ...
          <tr >
           ...
           <td class='<%=class_style%>' ><%=rs("name")%><%if rs("image")<>"" then%>&nbsp;[<span style="color: #FF0000">图</span>]<%end if%></td>
			...
			sql="select name from en_web_menu_type where [id]="&rs("position")&""
			rst.open(sql),cn,1,1
            ...
            <div align="center"><a href="en_menu_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_menu_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
		  next
...
sqlClass1="select id,name from en_web_menu_type  order by id" 
