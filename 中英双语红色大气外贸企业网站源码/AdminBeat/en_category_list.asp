...
sql="select id,pid,ppid,name,folder,ClassType,[order] from en_category where ppid=1 order by [order],time"
rs.open(sql),cn,1,1
...
for p_i=1 to loopno
          ...
          <tr >
			...
			一级栏目            </div></td>
            <td class="TitleHighlight2" >
            <div align="center"><a href="en_category_add.asp?pid_name=<%=rs("name")%>&pid=<%=rs("id")%>&ppid=2&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">添加二级栏目</a> | <a href="en_category_edit.asp?id=<%=rs("id")%>&ppid=1&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">栏目设置</a> | <a href="<%=ListName%>?cid=<%=rs("id")%>&act=search">内容管理</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_category_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除栏目</a>            </div></td>
          </tr>
...
if not rs2.eof and not rs2.bof then
		  ...
		  %>
            ...
            <td width="42%" class="TitleHighlight3"  >
              <div align="center"><a href="en_category_add.asp?pid_name=<%=rs("name")%>&pid_name2=<%=rs2("name")%>&pid=<%=rs2("id")%>&ppid=3">添加三级栏目</a> | <a href="en_category_edit.asp?id=<%=rs2("id")%>&pid_name=<%=rs("name")%>&ppid=2">栏目设置</a> | <a href="<%=ListName%>?pid=<%=rs2("id")%>&act=search">内容管理</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_category_del.asp?id=<%=rs2("id")%>';">删除栏目</a> </div>			  </td></tr>
