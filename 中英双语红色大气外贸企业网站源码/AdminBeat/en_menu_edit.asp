...
sql="select * from en_web_menu where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("url")=l_url
...
response.Write "<script language='javascript'>alert('修改成功！');location.href='en_menu_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from en_web_menu where id="&article_id&""
rs.open(sql),cn,1,1
	 ...
	 <tr>
	    ...
	    <td class='forumRowHighLight'><label>
	      <select name="position" id="position">
		   ...
		   sql="select id,name from en_web_menu_type order by [order]"
		   rsp.open(sql),cn,1,1
