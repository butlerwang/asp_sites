...
%>
	...
	<a href="learn.asp?typeid=<%=my_rs("id")%>">
         <FONT COLOR="red"><B><%=my_rs("type")%></B></FONT></a>
	<%else%>	  <a href="learn.asp?typeid=<%=my_rs("id")%>">
          <%=my_rs("type")%></a><br> 
     <%end if%>        
      <%my_rs.movenext 
loop 
...
%> <table border="0" cellspacing="0" cellpadding="0" width="100%">
              <tr bgcolor="#303430"> <form method=Post action="learn.asp"><FONT COLOR="#ffffff">
               [<b><%=rs.pagecount%></b>/<%=page%>页] [共<%=totalfilm%>个] <%if page=1 then%> [首页] [上一页] <% else %> [<a href="learn.asp?page=1&typeid=<%=typeid%>">首页</a>] 
               [<a href="learn.asp?page=<%=page-1%>&typeid=<%=typeid%>">上一页</a>]<%end if%><%if rs.pagecount-page<1 then%> [下一页] [尾页]  <%else%> [<a href="learn.asp?page=<%=page+1%>&typeid=<%=typeid%>">下一页</a>]  [<a href="learn.asp?page=<%=rs.pagecount%>&typeid=<%=typeid%>">尾页</a>]</FONT> <%end if%> <input type='text' name='page' size=2 maxlength=10 style="font-size:9pt;color:#FFFFFF;background-color:#666666;border-left: 1px solid #000000; border-right: 1px solid #000000; border-top: 1px solid #000000; border-bottom: 1px solid #000000" value="<%=page%>" align=center> <INPUT TYPE="hidden" name=type value="<%=typeid%>"><input style="border:1 solid black;FONT-SIZE: 9pt; FONT-STYLE: normal; FONT-VARIANT: normal; FONT-WEIGHT: normal; HEIGHT: 18px; LINE-HEIGHT: normal" type='submit'  value=' Goto '   size=2></td>
             </tr>
           </table>   
        </td>                                   
    </tr>                            
...
%>                                                         
    <tr>                
      ...
      <td bgcolor="#F6F6F6" width="60%" colspan="2">  <a href="showDetail.asp?id=<%=rs("id")%>" onClick="return js_openpage(this.href);"><%=rs("title")%>                                                                                      </A>                                      
                　</td>                                   
...
loop 
 ...
 <form  name="sele"  method="post"  action="learn.asp"  onsubmit="return  ckse()">              
  <tr >                            
