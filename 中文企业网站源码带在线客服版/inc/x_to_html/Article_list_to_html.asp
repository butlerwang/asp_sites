...
<%
 '文章列表读取替换
sql="select [id] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
sql="select [id],[title],[content],[url],[file_path],[time] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [time] desc"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
...
list_block=list_block&"<li class='ColorLink'><span>"&year(rs("time"))&"-"&month(rs("time"))&"-"&day(rs("time"))&"</span><a href='"&rs_url&"' target='_blank'>"&left(rs("title"),40)&"</a>"
list_block=list_block&"<p >"&left(ClearHtml(rs("content")),100)&" ...</p>"
