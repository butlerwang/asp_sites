...
if comment="" then
response.Write "<script language='javascript'>alert('Please input your message');history.go(-1)</script>"
else

    if request("verycode")="" then
    response.write "<script language=javascript>alert('Verification code is wrong^_^');history.go(-1);</script>"
  	Response.End 
...
rs.open(sql),cn,1,3
if not rs.eof then  
response.Write "<script language='javascript'>alert('Don't resent your message');history.go(-1)</script>"
else
rs.addnew
if article_id<>"" then
...
rs_1.close
set rs_1=nothing
			response.write"<SCRIPT language=JavaScript>alert('Your message is sent^_^');"
