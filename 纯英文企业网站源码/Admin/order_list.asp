...
if act="search" then
if keywords<>"" then
s_sql="select * from web_order where [content] like '%"&keywords&"%'   order by [time] desc "
else
s_sql="select * from web_order where [content] like '%"&keywords&"%'  order by [time] desc"
end if
else
...
%>
          ...
          <tr >
			...
			set rst=nothing
			%></td>
            <td class='forumRowHighLight'><div align="center"><a href="order_view_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已处理<%else%><span style="color: #FF0000">未处理</span><% end if%></a></div></td>
            <td class='forumRowHighLight' ><div align="center"><%=rs("time")%></div></td>
            <td class='forumRowHighLight' >
            <div align="center"><a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='order_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a></div>				</td>
          </tr>
		            <tr >
            <td  colspan="5" valign="top" class='forumRow'  style="line-height:200%"><span style="color: #060">联系人：</span><%=rs("name")%>&nbsp;&nbsp;<br><span style="color: #060">联系地址：</span><%=rs("address")%>&nbsp;&nbsp;<br><span style="color: #060">联系电话：</span><%=rs("tel")%>&nbsp;&nbsp;<br><span style="color: #060">电子邮件：</span><%=rs("email")%>&nbsp;&nbsp;<br><span style="color: #060">IP：</span><%=rs("IP")%>
