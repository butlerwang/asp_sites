...
%>
          <tr >
            ...
            <td class='<%=class_style%>' >&nbsp;<a href="<%=Model_FolderName&"/"&rs("file_path")%>" target="_blank"><%=left(rs("title"),35)%></a><%if rs("image")<>"" then%>&nbsp;[<span style="color: #FF0000">图</span>]<%end if%><%if rs("index_push")=1 then%>&nbsp;[<span style="color: #FF0000">荐</span>]<%end if%><%if rs("slide_yes")=1 then%>&nbsp;[<span style="color: #FF0000">幻灯</span>]<%end if%><%if rs("special_yes")=1 then%>&nbsp;[<span style="color: #FF0000">专题</span>]<%end if%></td>
