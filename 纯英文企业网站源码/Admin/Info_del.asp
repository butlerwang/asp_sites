...
sql="select id,cid,file_path from web_info where id="&cint(article_id(i))&""
rs.open(sql),cn,1,3
...
next
			...
			set rs=server.createobject("adodb.recordset")
sql="select id,cid,file_path from web_info where id="&article_id&""
rs.open(sql),cn,1,3
FilePath=rs("file_path")
ClassID=rs("cid")
rs.delete
...
response.Write "<script language='javascript'>alert('删除成功！');location.href='info_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
			%></td>
          </tr>
        </table>
	    </td>
