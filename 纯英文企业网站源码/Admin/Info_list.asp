...
sqlClass2="select id,pid,ppid,name from web_cate where ppid=2 and ClassType=1 order by id " 
...
sqlClass3="select id,pid,ppid,name from web_cate where ppid=3 and ClassType=1 order by id" 
...
%>
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="info_add.asp">添加新的职位</a></td>
          </tr>
          
      </table>
 <form name="form2" method="post" action="info_Del.asp?action=AllDel&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">
...
%>
          <tr >
            ...
            <td class='<%=class_style%>' >&nbsp;<a href="info_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>" ><%=left(rs("title"),35)%></a></td>
            <td class='<%=class_style%>' ><div align="center"><a href="info_view_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已审核<%else%><span style="color: #FF0000">未审核</span><% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><%=rs("time")%></div></td>
            <td class='<%=class_style%>' >
            <div align="center"><a href="info_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='info_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr>
		  		  <%
		  rs.movenext
		  next
