...
do while not rs.eof
	...
	WebNewNewsList=WebNewNewsList&"<td width='20%'><span>"&year(rs("time"))&"-"&month(rs("time"))&"-"&day(rs("time"))&"</span></td></tr>"
	rs.movenext
...
sql="select top 16 [title],file_path,[image] from [article] where ArticleType=2 and index_push=1 and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
Block03_LeftItem=Block03_LeftItem&"<DIV class='box'><A class='imgBorder'  href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><IMG alt='"&rsp("title")&"' src='/images/up_images/"&rsp("image")&"'>"&left(rsp("title"),14)&"</A> </DIV>"
rsp.movenext
loop
end if 
rsp.close
