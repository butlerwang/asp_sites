...
rss.open(sql),cn,1,1
if not rss.eof then
web_TopMenu=web_TopMenu&"<li id='aaabbb"&i&"' onmouseover=juhaoyongNavBgaColor('aaabbb"&i&"') onmouseout=style.background=''><a href='"&rs("url")&"' onmouseover=mopen('m"&i&"') onmouseout='mclosetime()'>"&rs("name")&"</a> "
...
HTML_product_order_show="<a onclick="&Chr(34)&"showPopWin('/order/?id="&a_id&"', 800, 500, null);"&Chr(34)&" href='#'><img src=/css/"&web_theme&"/order_img.gif alt='提交订单' border=0></a>&nbsp;&nbsp;&nbsp;&nbsp;"
else
...
juhaoyongMoreCid=rs("cid")'用于更多查询
...
sql="select top 6 [title],file_path,[image] from [article] where ArticleType=2 and cid='"&juhaoyongMoreCid&"' and id<>"&a_id&" and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
MorePro_List=MorePro_List&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&trim(rsp("title"))&"'><img src='/images/up_images/"&rsp("image")&"' width='139' height='139'/><div class='albumtitle'>"&left(rsp("title"),16)&"</div></a></div></div>"
rsp.movenext
loop
else
