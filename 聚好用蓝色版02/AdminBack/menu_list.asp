...
%>
	...
	<tr>
	  <th width="100%" height=25 class='tableHeaderText'>二级导航</th>
...
<p>2、不要设置过多导航，可能会导致页面变形。</p>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="menu_add.asp">添加新的二级导航</a></td>
          </tr>
          <tr>
            <td height="30"></td>
          </tr>
...
class_style="forumRowHighLight"
          ...
          <tr >
           ...
           <td class='<%=class_style%>' ><%=rs("name")%><%if rs("image")<>"" then%>&nbsp;[<span style="color: #FF0000">图</span>]<%end if%></td>
            ...
            <div align="center"><a href="menu_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，确定要删除吗？')) location.href='menu_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
		  next
