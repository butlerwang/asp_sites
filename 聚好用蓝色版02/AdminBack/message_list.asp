...
class_style="forumRowHighLight"
          ...
          <tr >
            ...
            <div align="center"><a  href="#"   onclick = "show('fd_<%=rs("id")%>');return false;">查看</a> | <a href="message_reply.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">回复</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，确定要删除吗？')) location.href='message_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a></div>
