...
sql="select * from web_ads_position where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("memo")=l_memo
rs.update
...
response.Write "<script language='javascript'>alert('修改成功！');location.href='ads_position_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from web_ads_position where id="&article_id&""
rs.open(sql),cn,1,1
...
return true;}
	...
	<tr>
	  <th class='tableHeaderText' colspan=2 height=25>修改在线客服</th>
	<tr>
	<td width="15%" height=23 class='forumRowHighLight'>在线客服名称 (必填) </td>
	<td width="85%" class='forumRowHighLight'><input name='name' type='text' id='name'  value="<%=rs("name")%>"size='70'>
	 <input name='l_id' type='hidden' id='l_id' value="<%=rs("id")%>" size='70'>
	  ...
	  <td class='forumRow' height=11>在线客服代码</td>
