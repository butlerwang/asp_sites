...
sql="select top 1 [id],cid,pid,ppid from [Article] where [title]='"&a_title&"' and ArticleType=2 order by [time] desc"
rs2.open(sql),cn,1,1
if not rs2.eof  then
...
response.Write "<script language='javascript'>alert('添加成功！');location.href='Product_list.asp?ppid="&juhaoyong_ppid&"&act=search';</script>"
elseif juhaoyong_pid>0 then
response.Write "<script language='javascript'>alert('添加成功！');location.href='Product_list.asp?pid="&juhaoyong_pid&"&act=search';</script>"
elseif juhaoyong_cid>0 then
response.Write "<script language='javascript'>alert('添加成功！');location.href='Product_list.asp?cid="&juhaoyong_cid&"&act=search';</script>"
end if
...
%>
	  ...
	  <tr>
	    ...
	    <td class='forumRow'><input name='SalePrice' type='text' id='SalePrice' size='30'><font color="#FF0000">（若为空，则前台详情页不显示该项）</font>
        </td>
      </tr>
	  <tr>
	    <td class='forumRowHighLight' height=23>型号</td>
	    <td class='forumRowHighLight'><input name='SaleCount' type='text' id='SaleCount' size='30'><font color="#FF0000">（若为空，则前台详情页不显示该项）</font></td>
      </tr>
	  <tr>
	    <td class='forumRow' height=23>市场价</td>
	    <td class='forumRow'><input name='a_wine' type='text' id='a_wine' size='30'><font color="#FF0000">（若为空，则前台详情页不显示该项）</font></td>
      </tr>
	  <tr>
	    <td class='forumRowHighLight' height=23>优惠价</td>
	    <td class='forumRowHighLight'><input name='a_net' type='text' id='a_net' size='30'><font color="#FF0000">（若为空，则前台详情页不显示该项）</font></td>
      </tr>      
	  <tr>
	    ...
	    <td width="85%" class='forumRowHighLight'><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
           ...
           <td width="78%"  ><iframe width="500" name="ad" frameborder=0 height=30 scrolling=no src=upload.asp></iframe><font color="#FF0000">（注：上传的图片名称必须是英文或数字）</font></td>
         </tr>
       </table></td>
      </tr>
	...
	<tr>
	  <td class='forumRowHighLight' height=23>添加者</td>
	...
	<tr>
	  ...
	  <td class='forumRowHighLight'><span class="forumRow">
	    <input name='product_tbbuy_url' type='text' id='product_tbbuy_url' size='120'>
	...
	<tr>
	  ...
	  <td class='forumRowHighLight'><input type="radio" name="a_index_push" value="1">是&nbsp;<input name="a_index_push" type="radio" value="0" checked>否</td>
	</tr>
