...
s_sql="select id,title,cid,pid,ppid,file_path,from_name,SalePrice,image,index_push,slide_yes,special_yes,view_yes,headline,hit,ip,time from Article where [title] like '%"&keywords&"%'   and ArticleType=2 order by time desc"
elseif pid="" and ppid="" then
search_sql="and cid='"&cid&"'"
s_sql="select id,title,cid,pid,ppid,file_path,from_name,SalePrice,image,index_push,slide_yes,special_yes,view_yes,headline,hit,ip,time from Article where [title] like '%"&keywords&"%'"&search_sql&" and ArticleType=2 order by time desc"
elseif ppid="" then
...
s_sql="select id,title,cid,pid,ppid,file_path,from_name,SalePrice,image,index_push,slide_yes,special_yes,view_yes,headline,hit,ip,time from Article where ArticleType=2 order by time desc"
...
%>
	...
	<tr>
	  <th width="100%" height=25 class='tableHeaderText'>产品列表</th>
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td height="30" valign="top" class="TipWords"><p>1、产品列表显示您所添加的所有产品，标示“未审核”的产品将不会在网站中显示。</p>
                <p>2、删除产品将会同步删除数据库中的记录和产品的具体地址请慎重。</p>
            </td>
          </tr>
          <tr>
            <td height="10" ></td>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="Product_add.asp?juhaoyong_cid=<%=juhaoyong_cid%>&juhaoyong_pid=<%=juhaoyong_pid%>&juhaoyong_ppid=<%=juhaoyong_ppid%>">新增添加内容</a></td>
          </tr>
 ...
 <form name="form2" method="post" action="Product_Del.asp?action=AllDel&juhaoyong_cid=<%=juhaoyong_cid%>&juhaoyong_pid=<%=juhaoyong_pid%>&juhaoyong_ppid=<%=juhaoyong_ppid%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="2">
          <tr>
            ...
            <td width="29%" height="30" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">产品标题</div></td>
            <td width="17%" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">产品分类</div></td>
            <td width="9%" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">品牌</div></td>
...
%>
          <tr >
            ...
            <td class='<%=class_style%>' >&nbsp;<a href="<%=Model_FolderName&"/"&rs("file_path")%>" target="_blank"><%=left(rs("title"),18)%></a><%if rs("image")<>"" then%>&nbsp;[<span style="color: #FF0000">图</span>]<%end if%><%if rs("index_push")=1 then%>&nbsp;[<span style="color: #FF0000">荐</span>]<%end if%><%if rs("slide_yes")=1 then%>&nbsp;[<span style="color: #FF0000">幻灯</span>]<%end if%><%if rs("special_yes")=1 then%>&nbsp;[<span style="color: #FF0000">专题</span>]<%end if%></td>
            ...
            <td class='<%=class_style%>' ><div align="center"><a href="Product_view_yes.asp?id=<%=rs("id")%>&juhaoyong_cid=<%=juhaoyong_cid%>&juhaoyong_pid=<%=juhaoyong_pid%>&juhaoyong_ppid=<%=juhaoyong_ppid%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已审核<%else%><span style="color: #FF0000">未审核</span><% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><%=rs("time")%></div></td>
            <td class='<%=class_style%>' >
            <div align="center"><a href="Product_edit.asp?id=<%=rs("id")%>&juhaoyong_cid=<%=juhaoyong_cid%>&juhaoyong_pid=<%=juhaoyong_pid%>&juhaoyong_ppid=<%=juhaoyong_ppid%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，确定要删除吗？')) location.href='Product_del.asp?id=<%=rs("id")%>&juhaoyong_cid=<%=juhaoyong_cid%>&juhaoyong_pid=<%=juhaoyong_pid%>&juhaoyong_ppid=<%=juhaoyong_ppid%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr>
		  		  <%
		  rs.movenext
		  next
...
response.write "<div align='center'><span style='color: #FF0000'>暂无产品！</span></div>"
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| 产品搜索</td>
          </tr>
          <tr>
