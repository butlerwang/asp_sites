...
class_style="forumRowHighLight"
          ...
          <tr >
           ...
           <td class='<%=class_style%>' ><div align="center"><%=rs("dataname")%></div></td>
            ...
            <div align="center"><a href="Data_Restore.asp?Dataname=<%=rs("dataname")%>">还原</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，确定要删除吗？')) location.href='Data_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
		  next
