...
'问吧模板类型获取
set rs_1=server.createobject("adodb.recordset")
sql="select FileName,FolderName from web_Models_type where [id]=41"
rs_1.open(sql),cn,1,1
...
'侧边栏当前栏目列表
set rs=server.createobject("adodb.recordset")
sql="select [id],[name],[folder] from [en_category] where ClassType=5 and ppid=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
ClassID=rs("id")
ClassName1=rs("name")
...
<%
 '列表读取替换
sql="select [id] from en_web_article_comment where view_yes=1 and article_id=0  order by [time]"
...
rs_order=1
sql="select * from en_web_article_comment where view_yes=1 and article_id=0  order by [time]"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
...
%>
<%
filepath="/English"&Model_FolderName&"/list_"&j&".html"
filepath_index="/English"&Model_FolderName&"/index.html"
%>
<%
if j>1 then
'读取模板
