...
'列表模板类型获取
set rs_1=server.createobject("adodb.recordset")
sql="select FileName,FolderName from web_Models_type where [id]=42"
rs_1.open(sql),cn,1,1
...
Block_LeftClassList=Block_LeftClassList&"<li class='current'><A href='/English/"&ClassFolder1&"/"&rsl("Folder")&"'>"&rsl("name")&"</A> "
else
...
do while not rs.eof 
Block_LeftClassList=Block_LeftClassList&"<li><a href='/English/"&ClassFolder1&"/"&rsl("Folder")&"/"&rs("folder")&"/' >"&rs("name")&"</a></li> "
rs.movenext
loop
Block_LeftClassList=Block_LeftClassList&"</ul>"
end if
...
<%
 '文章列表读取替换
sql="select * from [en_article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=2 order by [time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
list_block=list_block&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank'  title='"&left(rs("title"),39)&"'><img src='/images/up_images/"&rs("image")&"' title='"&left(rs("title"),39)&"'/></a><div class='albumtitle'><a href='"&rs_url&"' target='_blank'  title='"&left(rs("title"),39)&"'>"&left(rs("title"),22)&"</a></div></div></div>"
...
<%'循环列表替换内容
replace_code=replace(replace_code,"$list_block$","No Products.")   
