...
for ii=0 to dc
Article_Downloads=Article_Downloads&"<div class='download ColorLink'><b>Download File：</b>"
Article_Downloads=Article_Downloads&"<a href='/data/"&DownloadFiles(ii)&"' target='_blank'>"&DownloadFiles(ii)&"</a> <span class='ListDate'>(Click to download)</span></div> "
next
end if
...
'上一篇，下一篇读取替换
set rs=server.createobject("adodb.recordset")
sql="select top 1 [title],[file_path],url,[time] from [en_article] where [id]>"&a_id&"  and view_yes=1 and ArticleType=1 order by [id] "
rs.open(sql),cn,1,1
...
article_next=article_next&"None"
end if
...
set rs=server.createobject("adodb.recordset")
sql="select top 1 [title],[file_path],url,[time] from [en_article] where [id]<"&a_id&"  and view_yes=1 and ArticleType=1 order by [id] desc"
rs.open(sql),cn,1,1
