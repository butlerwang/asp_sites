...
'文章列表模板类型获取
set rs_1=server.createobject("adodb.recordset")
sql="select FileName,FolderName from web_Models_type where [id]=51"
rs_1.open(sql),cn,1,1
...
<%
 '文章列表读取替换
sql="select [id] from [en_article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=3 order by [time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
sql="select [title],[content],[url],[file_path],[image],pics from [en_article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=3 order by [time] desc"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
