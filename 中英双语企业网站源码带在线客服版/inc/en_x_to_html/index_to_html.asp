...
sql="select FileName,FolderName from web_Models_type where [id]=52"
rs_1.open(sql),cn,1,1
if not rs_1.eof and rs_1("FolderName")<>"" then
CasesContent_FolderName="/English/"&rs_1("FolderName")
end if
rs_1.close
set rs_1=nothing
...
'模板类型获取
set rs_1=server.createobject("adodb.recordset")
sql="select FileName,FolderName from web_Models_type where [id]=37"
rs_1.open(sql),cn,1,1
...
'首页顶部幻灯广告读取替换
set rs=server.createobject("adodb.recordset")
sql="select top 5 name,url,image,ADcode from en_web_ads  where [position]=36 and view_yes=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
if rs("adcode")<>"" then
...
'友情链接
set rs=server.createobject("adodb.recordset")
sql="select  [name],[url],[image],follow_yes from [en_web_link] where view_yes=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
do while not rs.eof
...
'企业介绍
set rs=server.createobject("adodb.recordset")
sql="select top 1  [name],[folder],[id],[pid],[ppid],[image],[content] from [en_category] where ClassType=5 and ppid=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
ItemID=rs("id")
...
WebAboutContent=left(ClearHtml(rs("content")),148)
...
'新闻动态
set rs=server.createobject("adodb.recordset")
sql="select top 1  [name],[folder],[id],[pid],[ppid] from [en_category] where ClassType=1 and ppid=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
ItemID=rs("id")
...
ClassSQL="pid"
set rs0=server.createobject("adodb.recordset")
sql="select [name],[folder],[id],[pid],[ppid] from [en_category] where id="&rs("pid")
rs0.open(sql),cn,1,1
if not rs0.eof then
...
if not rs0.eof then
set rs00=server.createobject("adodb.recordset")
sql="select [name],[folder],[id],[pid],[ppid] from [en_category] where id="&rs0("pid")
rs00.open(sql),cn,1,1
if not rs00.eof then
...
set rs=server.createobject("adodb.recordset")
sql="select  [name],[folder],[id],[pid],[ppid] from [en_category] where pid="&ItemID&" and ClassType=1 and ppid=2 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
WebNewNewsTitles=WebNewNewsTitles&"<li><A href='/English"&WebNewNewsFolder&"/"&rs("Folder")&"'>"&rs("name")&"</A></li> "

WebNewNewsList=WebNewNewsList&"<li><table class='MBlockTable' width='100%' border='0' cellspacing='0' cellpadding='0'>"
set rsp=server.createobject("adodb.recordset")
sql="select top 8 [title],file_path,[time] from [en_article] where ArticleType=1 and cid='"&ItemID&"' and pid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
...
'文章栏目列表
set rs=server.createobject("adodb.recordset")
sql="select top 2 [name],[folder],[id],[pid],[ppid] from [en_category] where id<>"&ItemID&" and ClassType=1 and ppid=1 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to 2
WebArticleTitles=WebArticleTitles&"<li><A href='/English/"&rs("Folder")&"'>"&rs("name")&"</A></li> "

WebArticleList=WebArticleList&"<li><div class='DivList'>"
set rsp=server.createobject("adodb.recordset")
sql="select top 7 [title],file_path,[time] from [en_article] where ArticleType=1 and cid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ArticleContent_FolderName&"/"&rsp("file_path")
WebArticleList=WebArticleList&"<div class='DivLi'>· <a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),36)&"</a></div>"
rsp.movenext
loop
end if 
rsp.close
...
'案例
set rs=server.createobject("adodb.recordset")
sql="select top 1  [name],[folder],[id],[pid],[ppid] from [en_category] where ClassType=3 and ppid=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
ItemID=rs("id")
WebCaseName=rs("name")
...
WebCaseList=WebCaseList&"<div class='topic'><div class='TopicTitle'><a  href='/English/"&WebCaseFolderName&"/"&"'>"&rs("name")&"</a></div>"
WebCaseList=WebCaseList&"<div class='TopicMore'> <a  href='/English/"&WebCaseFolderName&"'><img src='/images/more.png'></a></div>"
...
set rsp=server.createobject("adodb.recordset")
sql="select top 1 [title],file_path,[image] from [en_article] where ArticleType=3 and cid='"&ItemID&"' and [image]<>'' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
rs_url=""
rs_url=CasesContent_FolderName&"/"&rsp("file_path")
WebCaseList=WebCaseList&"<a href='"&rs_url&"' target='_blank'><img src='/images/Up_Images/"&rsp("image")&"' width='204' height='155' alt='"&rsp("title")&"'></a><p><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),30)&"</a></p>"
else
WebCaseList=WebCaseList&"No Case."
end if 
rsp.close
set rsp=nothing
...
set rsp=server.createobject("adodb.recordset")
sql="select top 7 [title],file_path,[time] from [en_article] where ArticleType=3 and cid='"&ItemID&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
...
'品牌产品
set rs=server.createobject("adodb.recordset")
sql="select top 1  [name],[folder],[id],[pid],[ppid] from [en_category] where ClassType=2 and ppid=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
ItemID=rs("id")
...
sql="select top 14 [name],[folder],[id],[pid],[ppid] from [en_category] where pid="&ItemID&" order by [order] "
rsl.open(sql),cn,1,1
...
ClassCount=0
set rsp=server.createobject("adodb.recordset")
sql="select id from [en_article] where ArticleType=2 and cid='"&ItemID&"'  and pid='"&rsl("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
...
Block_LeftClassList=Block_LeftClassList&"<A href='/English"&WebProductFolder&"/"&rsl("Folder")&"'>"&rsl("name")&" <span>("&ClassCount&")</span></A> "

set rs=server.createobject("adodb.recordset")
sql="select  [name],[folder] from [en_category] where ppid=3 and pid="&rsl("id")&"  order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
Block_LeftClassList=Block_LeftClassList&"<ul>"
do while not rs.eof 
Block_LeftClassList=Block_LeftClassList&"<li> "
Block_LeftClassList=Block_LeftClassList&"<a href='/English"&WebProductFolder&"/"&rsl("Folder")&"/"&rs("folder")&"/' target='_blank' >"&rs("name")&"</a>"
Block_LeftClassList=Block_LeftClassList&" </li> "
rs.movenext
loop
Block_LeftClassList=Block_LeftClassList&"</ul>"
...
'list
set rsp=server.createobject("adodb.recordset")
sql="select top 12 [title],file_path,[image],[content] from [en_article] where ArticleType=2 and cid='"&ItemID&"' and index_push=1  and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
...
Block03_LeftItem=Block03_LeftItem&"<p class='ProTitle'><strong><A  href='"&rs_url&"' target='_blank' >"&left(rsp("title"),19)&"</A></strong> </p>"
...
<%'声明HTML文件名,指定文件路径
filepath=Model_FolderName&"/English/index.html"
%>
