...
'热点产品
set rs=server.createobject("adodb.recordset")
sql="select top 1  [id] from [en_category] where ClassType=2 and ppid=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
NewsID=rs("id")
end if
...
set rs=nothing
set rs=server.createobject("adodb.recordset")
sql="select top 5 title,content,file_path,[url],time from [en_article]  where  cid='"&NewsID&"'  and view_yes=1  and ArticleType=2 order by [Hit] desc"
rs.open(sql),cn,1,1
if not rs.eof and not rs.bof then
...
Block01_LeftItem=Block01_LeftItem&"<dd>· <a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'>"&left(rs("title"),50)&"</a></dd>"
rs_0.close
set rs_0=nothing
rs.movenext
next
...
sql="select top 3 [title],file_path,[image] from [en_article] where ArticleType=2 and id<>"&a_id&" and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
MorePro_List=MorePro_List&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><img src='/images/up_images/"&rsp("image")&"'  ></a><div class='albumtitle'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),28)&"</a></div></div></div>"
rsp.movenext
loop
else
MorePro_List=MorePro_List&"No Information."
end if 
rsp.close
set rsp=nothing
%>
...
<% ' 文章内容读取替换
set rs=server.createobject("adodb.recordset")
sql="select * from [en_article] where [id]="&a_id&" and view_yes=1"
rs.open(sql),cn,1,1
if not rs.eof then
...
article_time=year(rs("edit_time"))&"/"&month(rs("edit_time"))&"/"&day(rs("edit_time"))
...
Article_Pics=Article_Pics&"<li><a class=""group1 zoomer tooltip""  href=""/images/up_images/"&PicsContent(i)&""" title="""&rs("title")&""" ><img src=""/images/up_images/"&PicsContent(i)&"""  /><span class=""zoom""></span></a></li>"
...
sql="select [id],[pid],[ppid],[name],[folder] from [en_category] where [id]="&rs("cid")&" and ppid=1"
...
if rs("pid")<>"" then
set rs_1=server.createobject("adodb.recordset")
sql="select [id],[pid],[ppid],[name],[folder] from [en_category] where [id]="&rs("pid")&" and ppid=2"
rs_1.open(sql),cn,1,1
...
sql="select [id],[pid],[ppid],[name],[folder] from [en_category] where [id]="&rs("ppid")&" and ppid=3"
...
'----------------------
if folder_name<>"" then
folder_path="/English"&MainClass_FolderName&"/"&folder_name&"/"
end if
category_position1="<a href='"&folder_path&"'>"&rs_0("name")&"</a>"

if folder_name_1<>"" then
folder_path="/English"&MainClass_FolderName&"/"&folder_name&"/"&folder_name_1&"/"
end if
category_position2=" > <a href='"&folder_path&"'>"&rs_1("name")&"</a>"

if folder_name_2<>"" then
folder_path="/English"&MainClass_FolderName&"/"&folder_name&"/"&folder_name_1&"/"&folder_name_2&"/"
end if
...
sql="select [name],[folder],[id],[pid],[ppid] from [en_category] where pid="&ClassID&" order by [order] "
rsl.open(sql),cn,1,1
Block_LeftClassList=Block_LeftClassList&"<ul id='suckertree1'>"
if not rsl.eof then
for i=1 to rsl.recordcount
Block_LeftClassList=Block_LeftClassList&"<li><A href='/English/"&ClassFolder1&"/"&rsl("Folder")&"'>"&rsl("name")&"</A> "
...
do while not rs.eof 
Block_LeftClassList=Block_LeftClassList&"<li><a href='/English/"&ClassFolder1&"/"&rsl("Folder")&"/"&rs("folder")&"/' target='_blank' >"&rs("name")&"</a></li> "
rs.movenext
loop
Block_LeftClassList=Block_LeftClassList&"</ul>"
end if
...
PageList=PageList&"Current：<span class='FontRed'>" & i+1 & "</span>/" & c+1 &"&nbsp;"
PageList=PageList&"<a href="&Article_FilePath&">" & "First" & "</a>"
select case i
case 0
PageList=PageList&"&nbsp;&nbsp;Pre&nbsp;&nbsp;"
case 1
PageList=PageList&"<a href="&Article_FilePath&">" & "Pre" & "</a>"
case else
PageList=PageList&"<a href="&i-1&Article_FilePath&">" & "Pre" & "</a>"
end select
for counter=0 to c
...
PageList=PageList&"<a href="&c&Article_FilePath&">" & "Last" & "</a></div>"
%>
...
<%'声明HTML文件名,指定文件路径
if i=0 then
filepath="/English"&Model_FolderName&"/"&Article_FilePath
else
filepath="/English"&Model_FolderName&"/"&i&Article_FilePath
end if
%>
<% '生成静态文件
'读取模板
