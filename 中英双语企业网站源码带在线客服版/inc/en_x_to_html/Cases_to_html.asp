...
<% ' 文章内容读取替换
set rs=server.createobject("adodb.recordset")
sql="select * from [en_article] where view_yes=1 and [id]="&a_id&""
rs.open(sql),cn,1,1
if not rs.eof then
...
'更多展示
set rs=server.createobject("adodb.recordset")
sql="select top 9 title,image,file_path from [en_article]  where  cid='"&ClassID&"' and ArticleType=3  and id<>"&a_id&" and view_yes=1 order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof and not rs.bof then
for i=1 to rs.recordcount
