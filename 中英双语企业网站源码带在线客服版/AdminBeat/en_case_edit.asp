...
%>
<%
response.Write "<script language='javascript'>alert('OK');location.href='en_case_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
...
%>  <form id="form1" name="form1" method="post" action="?act1=save&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">
	...
	<tr>
	  <th class='tableHeaderText' colspan=2 height=25>修改案例</th>
	<tr>
	  <td height=23 colspan="2" class='forumRow'><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td height="30" valign="top" class="TipWords"><p>1、本系统采用手动分页方法，如果想要对案例进行分页，只需在编辑器按钮栏点击<img src="images/inserthorizontalrule.gif" width="20" height="20" />图标，就会在编辑器内自动插入一条线做为分页标志，你不需要删除它，它将不会在案例中显示。</p>
            </td>
        </tr>
        <tr>
          <td height="10">&nbsp;</td>
...
set rsc2=server.createobject("adodb.recordset")
sqlClass2="select id,pid,ppid,name from en_category where ppid=2 and ClassType=3 and pid="&cint(rs("cid"))&" order by id" 
rsc2.open sqlClass2,cn,1,1
...
%>
			...
			if rs("pid")<>"" then				
set rsc3=server.createobject("adodb.recordset")
sqlClass3="select id,pid,ppid,name from en_category where ppid=3 and ClassType=3 and pid="&cint(rs("pid"))&" order by id" 
rsc3.open sqlClass3,cn,1,1
...
<tr>
	...
	<tr>
	  ...
	  <td class='forumRowHighLight'><input name='a_OrderCount' type='text' id='a_OrderCount' value="<%=rs("OrderCount")%>" size='40'>
      &nbsp;只能是数字,数值越小排名越靠前</td>
	  </tr>	<tr>
	  <td class='forumRow' height=23>案例推荐</td>
	...
	<tr>
	  <td class='forumRowHighLight' height=23>替换关键字</td>
	  <td class='forumRowHighLight'><input type="radio" name="a_keywords_yes" value="1" >
是
      &nbsp;
