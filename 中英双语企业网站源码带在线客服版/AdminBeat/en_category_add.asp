...
sql="select * from en_category where [folder]='"&c_folder&"'"
rs.open(sql),cn,1,3
if not rs.eof then
...
set rs=server.createobject("adodb.recordset")
sql="select [folder] from [en_category] where [name]='"&pid_name&"'"
rs.open(sql),cn,1,1
if not rs.eof and not rs.bof then
folder_0=rs("folder")
Set Fso=Server.CreateObject("Scripting.FileSystemObject") 
If Fso.FolderExists(Server.MapPath("/English/"&rs("folder")))=false Then
NewFolderDir="/English/"&rs("folder")
call CreateFolderB(NewFolderDir)
end if


set rs2=server.createobject("adodb.recordset")
sql="select [folder] from [en_category] where [name]='"&pid_name2&"'"
rs2.open(sql),cn,1,1
if not rs2.eof and not rs2.bof then
folder_2=rs2("folder")
Set Fso=Server.CreateObject("Scripting.FileSystemObject") 
If Fso.FolderExists(Server.MapPath("/English/"&rs("folder")&"/"&rs2("folder")))=false Then
NewFolderDir="/English/"&rs("folder")&"/"&rs2("folder")
call CreateFolderB(NewFolderDir)
end if
...
if ppid=1 then
NewFolderDir="/English/"&c_folder
end if

if ppid=2 then
NewFolderDir="/English/"&folder_0&"/"&c_folder
end if

if ppid=3 then
NewFolderDir="/English/"&folder_0&"/"&folder_2&"/"&c_folder
end if
...
set rs2=server.createobject("adodb.recordset")
sql="select top 1 [id] from [en_category] where [name]='"&c_name&"' order by [time] desc"
rs2.open(sql),cn,1,1
if not rs2.eof  then
ClassID=rs2("id")
...
<%
response.Write "<script language='javascript'>alert('添加成功！');location.href='en_category_list.asp';</script>"
end if
end if
rs_f.close
set rs_f=nothing
...
response.write "当前为一级栏目"
	...
	<tr>
	  ...
	  <td class='forumRow'><label>
        ...
        <% if request.querystring("ClassTypeID")=1 then
		response.write "checked=""checked"""
		end if
		if request.querystring("ClassTypeID")="" then
		response.write "checked=""checked"""
		end if		
		%>
        ...
        <input name="ClassType" type="radio" value="2"
                <% if request.querystring("ClassTypeID")=2 then
		response.write "checked=""checked"""
		end if
		%> >
      产品
	    &nbsp;&nbsp;
        <input name="ClassType" type="radio" value="3" 
                <% if request.querystring("ClassTypeID")=3 then
		response.write "checked=""checked"""
		end if
		%>>
      案例
      ...
      <input name="ClassType" type="radio" value="4" 
	        <% if request.querystring("ClassTypeID")=4 then
		response.write "checked=""checked"""
		end if
		%> >
      招聘	  
...
<input name="ClassType" type="radio" value="5" 
        <% if request.querystring("ClassTypeID")=5 then
		response.write "checked=""checked"""
		end if
		%>>      
单页</label></td>
