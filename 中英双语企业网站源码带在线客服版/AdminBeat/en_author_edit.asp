...
sql="select * from en_web_article_author where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("url")=l_url
rs("image")=l_image
...
set rs=nothing
call index_to_html()
response.Write "<script language='javascript'>alert('修改成功！');location.href='en_author_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from en_web_article_author where id="&article_id&""
rs.open(sql),cn,1,1
