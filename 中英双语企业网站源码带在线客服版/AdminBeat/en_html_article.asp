...
sql="select [id],[ArticleType] from [en_article] where "&a_c_sql&" and "&a_t_sql&" and view_yes=1 order by [time] desc"
end if
if a_c_sql<>"" and a_t_sql="" then
sql="select [id],[ArticleType] from [en_article] where "&a_c_sql&"  and view_yes=1 order by [time] desc"
end if
if a_c_sql="" and a_t_sql<>"" then
sql="select [id],[ArticleType] from [en_article] where "&a_t_sql&"  and view_yes=1 order by [time] desc"
end if
else
...
sqlClass1="select id,pid,ppid,name from en_category where ppid=1  order by id" 
