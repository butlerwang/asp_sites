...
s_sql="select id,title,cid,pid,ppid,file_path,from_name,SalePrice,image,index_push,slide_yes,special_yes,view_yes,headline,hit,ip,time from en_Article where [title] like '%"&keywords&"%'   and ArticleType=2 order by time desc"
elseif pid="" and ppid="" then
search_sql="and cid='"&cid&"'"
s_sql="select id,title,cid,pid,ppid,file_path,from_name,SalePrice,image,index_push,slide_yes,special_yes,view_yes,headline,hit,ip,time from en_Article where [title] like '%"&keywords&"%'"&search_sql&" and ArticleType=2 order by time desc"
elseif ppid="" then
...
s_sql="select id,title,cid,pid,ppid,file_path,from_name,SalePrice,image,index_push,slide_yes,special_yes,view_yes,headline,hit,ip,time from en_Article where ArticleType=2 order by time desc"
...
%>
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="en_Product_add.asp">添加新的产品</a></td>
          </tr>

      </table>
 <form name="form2" method="post" action="en_Product_Del.asp?action=AllDel&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">
...
<% '产品列表模块
strFileName="en_Product_list.asp" 
pageno=25
...
%>
          <tr >
            ...
            <td class='<%=class_style%>' >&nbsp;<a href="<%=Model_FolderName&"/"&rs("file_path")%>" target="_blank"><%=left(rs("title"),38)%></a><%if rs("image")<>"" then%>&nbsp;[<span style="color: #FF0000">图</span>]<%end if%><%if rs("index_push")=1 then%>&nbsp;[<span style="color: #FF0000">荐</span>]<%end if%><%if rs("slide_yes")=1 then%>&nbsp;[<span style="color: #FF0000">幻灯</span>]<%end if%><%if rs("special_yes")=1 then%>&nbsp;[<span style="color: #FF0000">专题</span>]<%end if%></td>
            ...
            <td class='<%=class_style%>' ><div align="center"><a href="en_Product_view_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已审核<%else%><span style="color: #FF0000">未审核</span><% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><%=rs("time")%></div></td>
            <td class='<%=class_style%>' >
            <div align="center"><a href="en_Product_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_Product_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr>
		  		  <%
		  rs.movenext
		  next
