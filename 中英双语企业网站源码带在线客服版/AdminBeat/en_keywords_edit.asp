...
sql="select id from en_web_keywords where [name]='"&l_name&"' and id<>"&l_id&""
rs.open(sql),cn,1,3
if not rs.eof then
response.Write "<script language='javascript'>alert('该关键词已经存在！');history.go(-1);</script>"
response.End()
else
rs.close 
set rs=nothing
...
set rs=server.createobject("adodb.recordset")
sql="select * from en_web_keywords where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("url")=l_url
rs("memo")=l_memo
...
rs.close
set rs=nothing
response.Write "<script language='javascript'>alert('修改成功！');location.href='en_keywords_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from en_web_keywords where id="&article_id&""
rs.open(sql),cn,1,1
