...
<!-- #include file="page_nextu.asp" -->
<% '搜索模块
act=request.querystring("act")
keywords=trim(request.form("keywords"))
if act="search" then
if keywords<>"" then
s_sql="select * from en_web_article_comment  where [name] like '%"&keywords&"%' and article_id=0 order by time desc"
else
s_sql="select * from en_web_article_comment where  article_id=0 order by time desc"
end if
else
s_sql="select * from en_web_article_comment where  article_id=0 order by time desc"
end if
...
Call header()
%>
 <form name="form2" method="post" action="en_Message_Del.asp?action=AllDel&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="2">
          <tr>
            ...
            <td width="27%" height="30" class="TitleHighlight"><div align="center" style="font-weight: bold;color:#ffffff">留言人</div></td>
...
<% '文章列表模块
strFileName="en_message_list.asp" 
pageno=20
...
class_style="forumRowHighLight"
          ...
          <tr >
            ...
            <td class='<%=class_style%>' ><div align="center"><a href="en_Message_view_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已显示<%else%><span style="color: #FF0000">已隐藏</span><% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><%=rs("time")%></div></td>
            <td class='<%=class_style%>' >
            <div align="center"><a  href="#"   onclick = "show('fd_<%=rs("id")%>');return false;">查看</a> | <a href="en_message_reply.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">回复</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='en_message_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a></div>
...
<%else%>
<span style='color: #FF0000'>暂无回复！</span>&nbsp;&nbsp;<a href="en_message_reply.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">立即回复>>></a>
<%end if%></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
