...
sql="select [id] from en_category where ( [folder]='"&c_folder&"' and [folder]<>'' ) and [id]<>"&id2
...
sql="select * from en_category where id="&id2&""
rs.open(sql),cn,1,3
...
if ppid=1 then
NewFolderDir1="/English/"&c_folder
OldFolderDir="/English/"&c_folderold
end if

if ppid=2 then
NewFolderDir1="/English/"&folder_0&"/"&c_folder
OldFolderDir="/English/"&folder_0&"/"&c_folderold
end if

if ppid=3 then
NewFolderDir1="/English/"&folder_0&"/"&folder_2&"/"&c_folder
OldFolderDir="/English/"&folder_0&"/"&folder_2&"/"&c_folderold
end if
...
call Blank_Content_to_html(ClassID)
 ...
 %>
<%
response.Write "<script language='javascript'>alert('修改成功！');location.href='en_category_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if
end if
rs_f.close
set rs_f=nothing
...
<%
set rs2=server.createobject("adodb.recordset")
sql="select * from en_category where id="&id1&""
rs2.open(sql),cn,1,3
if not rs2.eof and not rs2.bof then
%>
...
response.write "当前为一级栏目"
    ...
    <td class='forumRowHighLight'><input type='text' id='c_folder' name='c_folder' value="<%=rs2("folder")%>" size='40'  >
      <span style="color: #FF0000">请使用英文命名，保持为空将自动随机数字命名,填入的字符将无效。勿与系统文件夹名重复</span> </td>
	</tr>
	  <tr>
