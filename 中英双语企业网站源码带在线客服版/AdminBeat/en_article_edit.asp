...
sql="select [web_theme],web_url from en_web_settings "
rs_1.open(sql),cn,1,1
if not rs_1.eof then
Theme_Folder=rs_1("web_theme")
web_url=rs_1("web_url")
end if
rs_1.close
set rs_1=nothing

set rs=server.createobject("adodb.recordset")
sql="select [name],[url] from en_web_keywords order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof  then
do while not rs.eof 

if rs("url")="" then
name_url=web_url&"/English/Search/index.asp?q="&rs("name")
else
name_url=rs("url")
end if
...
%>  <form id="form1" name="form1" method="post" action="?act1=save&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">
	...
	<tr>
	  <td height=23 colspan="2" class='forumRow'><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td height="30" valign="top" class="TipWords"><p>1、本系统采用手动分页方法，如果想要对文章进行分页，只需在编辑器按钮栏点击<img src="images/inserthorizontalrule.gif" width="20" height="20" />图标，就会在编辑器内自动插入一条线做为分页标志，你不需要删除它，它将不会在文章中显示。</p>
            </td>
        </tr>
        <tr>
          <td height="10">&nbsp;</td>
...
set rsc2=server.createobject("adodb.recordset")
sqlClass2="select id,pid,ppid,name from en_category where ppid=2 and ClassType=1 and pid="&cint(rs("cid"))&" order by id" 
rsc2.open sqlClass2,cn,1,1
...
%>
			...
			if rs("pid")<>"" then				
set rsc3=server.createobject("adodb.recordset")
sqlClass3="select id,pid,ppid,name from en_category where ppid=3 and ClassType=1 and pid="&cint(rs("pid"))&" order by id" 
rsc3.open sqlClass3,cn,1,1
...
%>
	  ...
	  <tr>
	    <td class='forumRowHighLight' height=23>上传文件</td>
	    <td width="85%" class='forumRowHighLight'><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
           <td ><input id="Files" name="Files" type="text" size="80" value="<%=rs("files")%>"/> <br><input type="button" value="上传文件" onClick="showUpload(null,'Files','',10,null);" />
		   </td>
         </tr>
       </table></td>
      </tr>
...
<tr>
	...
	<tr>
	  ...
	  <td class='forumRowHighLight'>
	   ...
	   <select name="position" id="position" onclick="document.form1.a_from_name.value=document.form1.position.value">
	      <option value="">请选择</option>
		   <% set rsp=server.createobject("adodb.recordset")
		   sql="select name from en_web_article_author order by [order] "
		   rsp.open(sql),cn,1,1
		   if not rsp.eof and not rsp.bof then
		   do while not rsp.eof 
		   %> <option value="<%=rsp("name")%>"  ><%=rsp("name")%></option>
	...
	<tr>
	  <td class='forumRowHighLight' height=23>文章推荐</td>
