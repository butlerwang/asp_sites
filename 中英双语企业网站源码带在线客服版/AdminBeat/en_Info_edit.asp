...
sql="select * from [en_web_info] where id="&a_id&""
rs.open(sql),cn,1,3
rs("title")=a_title
...
%>
<%
response.Write "<script language='javascript'>alert('修改成功！');location.href='en_info_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
%>
...
sqlClass2="select id,pid,ppid,name from [en_category] where ppid=2 order by id " 
...
sqlClass3="select id,pid,ppid,name from [en_category] where ppid=3 order by id" 
...
<%
			set rs=server.createobject("adodb.recordset")
sql="select * from [en_web_info] where id="&article_id&""
rs.open(sql),cn,1,1
...
sqlClass2="select id,pid,ppid,name from en_category where ppid=2 and ClassType=4 and pid="&cint(rs("cid"))&" order by id" 
...
sqlClass3="select id,pid,ppid,name from en_category where ppid=3 and ClassType=4 and pid="&cint(rs("pid"))&" order by id" 
